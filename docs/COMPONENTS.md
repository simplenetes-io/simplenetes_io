...
The goal for Simplenetes is to have as few moving parts as possible and robust and easy to understand design. You will not find any `iptables` nor any `etcd` cluster.
...
- Hosts
- Clusters and Cluster Projects (git repos)
...
The standalone `pod` shell script can be run as is locally or managed by the Simplenetes Daemon (simplenetesd) on a host.

The `pod` shell script uses `podman` to run containers, which is made for running containers root-less. Podman is compatible with Docker.

There are three special Pods in Simplenetes which most often are used in Clusters (but do not have to be):  
    - The Ingress pod: (inbound traffic routing and TLS termination coming from the interwebz (using haproxy))  
    - The Proxy pod: an internal traffic router so Pods can talk to other Pods on other hosts or on the same host within the internal network  
...
The `ProxyPod` above does not run any containers, it runs directly on the Host as a native application, but it is managed just as a Pod. However, you don't need to bother about that.
...
### Other Pod types
For example the Simplenetes Proxy is treated as a regular Pod, but is is not containerized because it accesses `.conf` files placed in the host root directory by the daemon.
...
Hosts are in our terminology divided into `loadbalancers` and `workers`. Loadbalancers are exposed to the public internet and workers are not but receive traffic from
the loadbalancers via the internal proxy.
...
Any pod could bind to the host network or map ports to the host interface to be puclically exposed, as long as the firewall rules allow the traffic.

Hosts which are workers are usually not exposed directly to the internet but receive traffic from the internal proxy which is then proxied to pods
running on the host according to the pod ingress rules.
...
A Cluster is a one or many hosts on the same VLAN (it can be as simple as your laptop)  
...
This is a design choice that the cluster is mirrored as the directory structure. We believe this brings a low mental burdon on grasping the system as a whole.

This also gives understandable, traceable GitOps procedures and so that you can inspect the full cluster layout in the git repo.
...
Setting up the Cluster with it's Virtual Machines is describe here [PROVISIONING.md](PROVISIONING.md)
...
A Pod which is open for traffic via the proxy declares a `clusterPort` in it's `expose` section in the `pod.yaml` file.
...
Note1: that there can be multiple replicas of a specific pod version listening to the same clusterPort, which means traffic will be shared a amongst them.  

Note2: when softly rolling out a new pod version the new version could also share the same clusterPort to gurantee no downtime because traffic is shared between prior and new pod versions until the old versions are removed.
...
Note4: for pods which receive traffic from the ingress by matching domain names and urls can do with automatically assigned cluster ports. Pods which are to serve internally need fixed cluster ports so that other pods know how to connect to them.

When a process inside a container wants to connect internally to another pod, it does that by opening a TCP socket to `proxy:clusterPort`. `proxy` is a static host name automatically put into each containers `/etc/hosts` file which points to the hosts internal IP address.
...
Note6: `clusterPorts` are a TCP ports which each proxy on each host is listening to. Connecting to a `clusterPort` (on a proxy) results in a connection from the proxy to another proxy and then to a mapped `hostPort` which connects to the `targetPort` inside the pod.

Cluster ports are often auto assigned but can be manually assigned in the range between `1024 and 65535`, however ports `30000-32767` are reserved for host ports and the proxy (which claims port 32767).  
...
## Daemon
The Simplenetes Daemon manages the lifecycle of all the Pods on the Hosts, regardless of their runtime type (be it podman or natively executable).
...
The Simplenetes Daemon is preferably installed with root priviligies so that it can create `ramdisks` for the Pods which require that, but it drops priviligies when it interacts with any pod script/executable.
...
## The sns management tool
The is the `simpletenes` repo which proivides the `sns` tool used to create and management a cluster.
