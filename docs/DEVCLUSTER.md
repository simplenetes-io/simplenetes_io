...
To manage a cluster use a _GNU/BusyBox/Linux_ (possibly _macOS_ or _Windows Subsystem for Linux_ will also do).

If you want to run the pods locally (acting as a host) then `podman` is mandatory and therefore a _GNU/Linux_ _OS_ is required.

A dev cluster works exactly as a production cluster, with the differences that:  

- There is only one host: your laptop or personal computer.
- The `simplenetesd` Daemon process is run in foreground in user mode and it is not installed as a systemd unit.
- TLS certificates cannot be issued, since it's a closed system and public DNS is not applicable. Self signed certificates can be used, though.
...
9.  Setting up a development work flow
...
- _cluster-id.txt_ which contains the cluster ID provided to `init-host`.
...
- _cluster-vars.env_ a key-value file which still is empty, but can have cluster-wide variables.
- _log.txt_ which is a log file storing all operation done on the cluster.

You can change the cluster ID at this point directly in _cluster-id.txt_, but doing it later is not recommended.

Now, let's create a Host which is not a Virtual Machine, but refers to our laptop instead. The option `--address=local` states that this is a local disk host.
...
- _host.env_, a key-value file containing the variables needed to connect to the host.
    The variable `HOSTHOME` dictates where the Host files will get copied to when syncing the cluster repo with the remote cluster. For hosts which have `HOST=local` set this is the directory on your laptop to where pods will get synced.
    This means that if simulating many Hosts on the same laptop they will need different `HOSTHOME` settings.
- _host.state_, a simple file which can contain the words `active`, `inactive` or `disabled`, and tells Simplenetes the state of this Host.
...
Initing the host will also install any Docker image registry _config.json_ file for that user as _~/.docker/config.json_. But doing this for local cluster will overwrite any existing such _config.json_ file for the local user, so no point doing this when running locally. See [REGISTRY.md](REGISTRY.md) for more info on image registries.  
...
Do not edit anything by hand inside the (pretend remote) _simplenetes/host-laptop_ directory. Changes are always to be synced to that directory from the cluster project using `sns`.

Note: On a remote Host we would also want to install the Simplenetes Daemon `(simplenetesd)`. The Daemon is the process which manages pod lifecycles according to the state the pod is set to have. This process is shown in the *PRODCLUSTER* section.
...
If you have another place for all your pods, you can set the `PODPATH` environment variable to point there.
...
Simplenetes will inform you about some variables defined in the _pod.env_ file which you *might* want to redefine in the `cluster-vars.env` file. Note that variables in the `pod.env` file must be prefixed with the pod name when put in the `cluster-vars.env` file.
...
All variables for pods which get defined in the cluster wide _cluster-vars.env_ file must be prefixed with the pod name. This serves to avoid clashes of common variable names, however variable names which are all `CAPS` are not to be prefixed because they are treated as globals. The convention on global variables is so that some variables could be shared between pods.
...
The _config_ dir from the pod repo will have been copied into the cluster repo as `./_config/podname/config`.
...
If all is good you will see a row appear as:
```
ports: 192.168.1.198:30000:80
```

This port mapping is what the proxy will connect to, but we will try to `curl` directly against it just to see that the pod is alive. We need to add `--haproxy-protocol` to curl because _NGINX_ is expecting the proxy protocol.  
...
Working with a dev-cluster you will either have the pods in `devmode=true` or not. Using `devmode` is typical when developing with single standalone pods which mount files directly from disk instead of having images built for them. When running your local dev-cluster you can choose to either simulate the full deal of building images for each pod update or having one or many pods in `devmode` and then not needing to build images for each iteration.

To put the pod in your dev-cluster to development mode set `simplenetes_io_devmode=true` in the _cluster-vars.env_ file prior to compiling the pod and syncing the cluster. Even though pods are synced to _host-laptop_, the pod will still correctly mount the disk since relative paths are translated into absolute paths when compiling.
...
If we now have a new version of the pod (as stated in the _pod.yaml_ file `podVersion`), we can compile and release it.  
...
If we are happy with our new release, we can then retire the previous version. In this case we *must* provide the pod version we want to retire, since the implicit option (default) is to operate on the latest release when no version parameter is provided.
...
Note that Simplenetes also supports transactional ways of doing rolling releases so we don't have to deal with all the details each time:  
...
A special thing about the Ingress pod is that it most often binds to the ports 80 and 443 on the host, but ports below 1024 are root only. This requires that the system is properly set up to allow for non-root users to bind to ports as low as 80 for the Ingress to work. `sns` sets this up automatically on hosts, but you will need to manually fix this on your computer.
...
As an option, the Ingress pod will also terminate TLS traffic.
...
Note that the Proxy pod is a "special" pod because it runs no containers. The Proxy pod is a native executable. However, since it adheres to the Pod API it is still treated and managed as a Pod.
